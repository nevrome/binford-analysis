...
```{r create first model, echo = TRUE}
initial_model <- lm(area ~ ., data = sel2)
```

```{r plot first model, warning=FALSE, fig.height=7, fig.cap="\\label{fig:initial_model_plot}Residuals vs Fitted: Streuung der Abweichungen von der Modellvohersage. Normal Q-Q: . Scale-Location: . Residuals vs Leverage: Einflussreiche Ausreißer. ^[Zur Beschreibung der diagnostischen Plots: http://data.library.virginia.edu/diagnostic-plots/ [16.8.2017]]"}
...
Abbildung \ref{fig:initial_model_plot} zeigt für dieses Modell die vier diagnostischen Standardplots, die die Funktion `plot.lm()` aus stats bereitstellt. 

```{r stewise multiple regression}
#MASS::stepAIC(model)
```
