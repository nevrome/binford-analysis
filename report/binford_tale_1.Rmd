...
Aus Tabelle \ref{final model estimate table} (und Gleichung \ref{eq:area_final}) wird ersichtlich, dass dieses Modell nun auf einer wesentlich diverseren Auswahl unabhängiger Variablen fußt. Abbildung \ref{plot model4} und Tabelle \ref{final model estimate table 2} belegen jedoch, dass es sich um ein solides Modell handelt. Den Kennwerten in Tabelle \ref{binford model estimate table 2} nach zu urteilen, scheint es wesentlich bessere Vorhersagen zu treffen als Binfords Modell. Ich möchte damit also den Reproduktionsversuch abschließen und die Ergebnisse diskutieren.
